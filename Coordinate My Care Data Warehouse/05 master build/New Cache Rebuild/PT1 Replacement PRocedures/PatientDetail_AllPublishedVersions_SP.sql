...
alter PROCEDURE [dbo].[AT__PatientDetail_AllPublishedVersions_SP] 
-- Amended for PD Upgrade
AS
BEGIN

IF OBJECT_ID('tempdb..#PublishedVersions') IS NOT NULL 
dROP TABLE #PublishedVersions

SELECT [GenusId]
      ,[VersionNumber]
      ,[PatientSummary]
      ,[CarePlan],
(select splitdata from (select splitdata,ROW_NUMBER() over (order by offset) as rn from dbo.SplitString(CarePlan,'|')) sel1 where rn=5) as CarePlanVersion,
(select splitdata from (select splitdata,ROW_NUMBER() over (order by offset) as rn from dbo.SplitString(PatientSummary,'|')) sel1 where rn=5) as PatientSummaryVersion

Into #PublishedVersions

from [ETL_PROD].[dbo].[CMC_PatientSummary_CarePlan] with (nolock)
...
IF OBJECT_ID('[ETL_Local_PROD].[dbo].[PatientDetail-AllPublishedVersions]') IS NOT NULL 
dROP TABLE [ETL_Local_PROD].[dbo].[PatientDetail-AllPublishedVersions]
...
-- Metadata
cp.DateLastSaved,
isnull(la.AuditId,cp.EnteredBy) as #EnteredBy,
-- only one version allowed for pre migration
isnull(la.ActionTime,cast(demo.Add_Date as datetime)) as Date_Assessment,
cp.LastApprovedBy as #ApprovedBy,
'Completed' as AssessmentStatus,
-- Correct approval timestamp MS 22.3.17
oap.Date_Approval as Date_Approval,
...
n.MiddleName,
-- deal with duff apostrophes MS 22.3.17
...
INTO [ETL_Local_PROD].[dbo].[PatientDetail-AllPublishedVersions]


from #PublishedVersions v
join ETL_PROD.dbo.CMC_CarePlan cp with (nolock) on v.CarePlan = cp.ItemId
join ETL_PROD.dbo.CMC_PatientSummary ps with (nolock)on v.PatientSummary = ps.ItemID
join ETL_PROD.dbo.CMC_Patient p with (nolock) on ps.Patient = p.ItemId
join ETL_PROD.dbo.CMC_Name n with (nolock) on p.Name = n.itemid
...
left join ETL_PROD.dbo.CMC_CarePackage pkg with (nolock) on pkg.ItemId = ps.CarePackage
left join ETL_PROD.dbo.CMC_Consent c with (nolock) on c.ItemId = p.Consent
left join ETL_PROD.dbo.CMC_CPR cpr with (nolock) on ps.CPR = cpr.ItemId
left join ETL_PROD.dbo.CMC_MedicalBackground mb with (nolock) on ps.MedicalBackground = mb.ItemId
left join ETL_PROD.dbo.CMC_MedicationSummary ms with (nolock) on ms.ItemId = ps.MedicationSummary
left join ETL_PROD.dbo.CMC_PatientSummary_PatientPreferences psf with (nolock) on ps.ItemId = psf.PatientSummary
left join ETL_PROD.dbo.CMC_PatientPreference pf with (nolock) on psf.PatientPreference = pf.ItemId
left join ETL_PROD.dbo.CMC_Prognosis pr with (nolock) on ps.Prognosis = pr.ItemId
...
-- Are there always only three CPR approvers?? WHo fucking knows, Mandy.
...
left join ETL_PROD.dbo.Coded_NamePrefix npc with (nolock) on npc.code = n.NamePrefix
...
----left join (select top 1 * from ETL_PROD.dbo.CMC_RUN_COMPLETE with (nolock) where ItemId=1) rc on 1=1
left join (
select cmc_id,add_date from Protocol.PatientDemographics with (nolock)
union all
select cmc_id,add_date from Protocol.PatientDemographicsMigratedDuplicates with (nolock)
...
left join (select apa.fromPatientSummary,ActionTime as Date_Approval,ROW_NUMBER()
over (PARTITION by frompatientsummary order by actiontime) as rn
from AuditPatient_New apa where ActionType = 'Publish') oap
on oap.frompatientsummary = v.PatientSummary and oap.rn=1
