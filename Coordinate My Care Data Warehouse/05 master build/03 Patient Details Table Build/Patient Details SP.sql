...
ALTER PROCEDURE [dbo].[AT_MASTERBUILD_PatientDetails_Build] 
...
--ALTER view [dbo].[Cache-WorkbaseDQInfo] as
	...
	if OBJECT_ID ('[ETL_Local_PROD].[dbo].[AT_WorkbaseDQInfo]') is not null
	drop table [ETL_Local_PROD].[dbo].[AT_WorkbaseDQInfo]
		select 
		[DeptKey]
      ,[DeptName]
      ,[OrganizationType]
      ...
      ,[DeptCloseDate]
		...
		--into [ETL_Local_PROD].[dbo].[AT_WorkbaseDQInfo]
		into [ETL_Local_PROD].[dbo].[AT_WorkbaseDQInfo]
...
-- Amended for PD Upgrade
 ...
 --select cast([Individual]+cast([DeptKey] as varchar(200)) as Nvarchar(400)) as DQKey,* into [ETL_Local_PROD].[dbo].[AT_AssessorDQInfo] from #AssessorDQInfo
   ...
   if OBJECT_ID ('Tempdb..#PatOA') is not null
	drop table #PatOA
select 
*
into #PatOA
...
select
CMC_ID,
ActionTime as Date_Original_Assessment,
ROW_NUMBER() over (PARTITION by cmc_id order by actiontime) as rn 
from [ETL_Local_PROD].[dbo].[AuditPatient_New] 
where ActionType = 'create')d where d.rn=1

 
--   if OBJECT_ID ('Tempdb..#PatOAP') is not null
--	drop table #PatOAP
--select 
--*
--into #PatOAP
...
--ROW_NUMBER() over (PARTITION by cmc_id order by actiontime) as rn 
--from [ETL_Local_PROD].[dbo].[AuditPatient_New] 
--where ActionType = 'Publish')d where d.rn=1
...
--Left join ETL_PROD.dbo.CMC_CarePlan cp1 on cp1.ItemId = a.FirstCarePlan  
  -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	cast(null as varchar(300)) as OriginalApproverJobTitle,
	cast(null as varchar(max)) as OriginalApproverWorkbase,
	cast(null as varchar(255)) as OriginalApproverWorkbaseODS,
	cast(null as varchar(max)) as Original_Approver_Prof_Group,
...
-- MS 18.3.17 Add original approver ODS code & role information
	...
	cast(null as varchar(max)) as Original_Approver_Role_Description,

	'Completed' as OriginalAssessmentStatus,
	oa.Date_Original_Assessment,
...
pri.DeptODSCode as PrognosisClinicianWorkbaseODS,
 ...
 into [ETL_Local_PROD].[dbo].[AT_Patient_General]
	...
	from [ETL_Local_PROD].[dbo].[AT_CarePlanVersion] a
Left join ETL_PROD.dbo.CMC_CarePlan cp on cp.ItemId = a.LastCarePlan  
Left join ETL_PROD.dbo.CMC_CarePlan cp1 on cp1.ItemId = a.FirstCarePlan  -----Originally entered by
left join AT_StaffDeptContext eb1 on eb1.ProviderOrgContext = cp1.EnteredBy
...
left join #PatOA oa on oa.CMC_ID = po.CMC_ID
--left join #PatOAP oap on oap.CMC_ID = po.CMC_ID
--select * from #PatOA
