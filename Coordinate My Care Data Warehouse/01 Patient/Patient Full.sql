...
/****** Object:  View [dbo].[Cache-PatientDetailFull]    Script Date: 14/10/2019 09:09:55 ******/
SET ANSI_NULLS ON
GO
...
--uu.Value as [TEMPLATE_UUID_CDA], --Excluded XML
...
ISNULL(ap.OriginalApproverWorkbase,mde.OriginalApproverWorkbase) as OriginalApproverWorkbase,
...
ISNULL(ap.OriginalApproverWorkbaseEid,mde.OriginalApproverWorkbaseEid) as OriginalApproverWorkbaseEId,
...
ISNULL(ap.LatestApproverWorkbase,mde.LatestApproverWorkbase) as LatestApproverWorkbase,
...
ISNULL(ap.LatestApproverWorkbaseEid,mde.LatestApproverWorkbaseEid) as LatestApproverWorkbaseEId,
...
ad.Value as ADDRESSES_CDA, --exclude XML
case when ad.Value is null then null else dbo.htmlStripper(ad.Value) end as ADDRESSES,--exclude XML
ci.Value as CONTACT_INFO_CDA,--exclude XML
case when ci.Value is null then null else dbo.htmlStripper(ci.Value) end as CONTACT_INFO,--exclude XML
aa.Value as ALIASES_CDA,--exclude XML
case when aa.Value is null then null else dbo.htmlStripper(aa.Value) end as ALIASES,--exclude XML
...
left join AllDataAddresses ad on ad.CMC_ID = pd.CMC_ID --Pateint address done
...
--left join AllDataContactInfo ci on ci.CMC_ID = pd.CMC_ID --exclude XML
...
--left join AllDataTemplateUUID uu on uu.CMC_ID = pd.CMC_ID Excluded xml
