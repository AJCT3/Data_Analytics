...
--if OBJECT_ID ('[ETL_Local_PROD].[dbo].[AT_CacheOrg]') is not null
--	drop table [ETL_Local_PROD].[dbo].[AT_CacheOrg]
...
---Add indexes to table -------------------------------------------------------------------------
  /**   
	 ALTER TABLE [ETL_Local_PROD].[dbo].[AT_CacheOrg]
		ADD CONSTRAINT PK_OrgID_ItemID PRIMARY KEY CLUSTERED (ItemID);

    CREATE INDEX ItemEnterprise
		ON [ETL_Local_PROD].[dbo].[AT_CacheOrg]  (ItemID, EnterpriseID);
 
	CREATE INDEX ODSEnterprise
		ON [ETL_Local_PROD].[dbo].[AT_CacheOrg]  (EnterpriseID, ODSCode);
 
 **/
------------------------------------------------------------------------------------------------- 

--select * into   [ETL_Local_PROD].[dbo].[AT_CacheOrgXXX] from CacheOrganization   order by itemID 
--select * from [ETL_Local_PROD].[dbo].[AT_CacheOrg]  order by itemID where ItemID not in (select itemid from CacheOrganization)


---Add indexes to table -------------------------------------------------------------------------
  /**   
	 ALTER TABLE [ETL_Local_PROD].[dbo].[AT_CacheOrgXXX]
		ADD CONSTRAINT ItemUnique PRIMARY KEY CLUSTERED (ItemID);

    CREATE INDEX ItemEnterprise
		ON [ETL_Local_PROD].[dbo].[AT_CacheOrgXXX]  (ItemID, EnterpriseID);
 
	CREATE INDEX ODSEnterprise
		ON [ETL_Local_PROD].[dbo].[AT_CacheOrgXXX]  (EnterpriseID, ODSCode);
...
group by Organization
	...
	--select * from  [ETL_Local_PROD].[dbo].[AT_CacheOrg] where source is null
	--select Count(*) from 
...
AND n.ItemId not like 'PS|%|%|%|%|%|LCA'
 ...
 ---Add indexes to table -------------------------------------------------------------------------
  /**   
	 ALTER TABLE [ETL_Local_PROD].[dbo].[AT_Dept]
		ADD CONSTRAINT ItemUnique PRIMARY KEY CLUSTERED (DeptKey);

    CREATE INDEX OrgEnterpriseDept
		ON [ETL_Local_PROD].[dbo].[AT_Dept] (Organization, DeptEnterpriseID);
 
	CREATE INDEX ODSEnterprise
		ON [ETL_Local_PROD].[dbo].[AT_CacheOrgXXX]  (EnterpriseID, ODSCode);
 
select top 500 * from  [ETL_Local_PROD].[dbo].[AT_Dept]  

 

ALTER TABLE
  [ETL_Local_PROD].[dbo].[AT_Dept]
ALTER COLUMN
  DeptKey
    bigint NOT NULL;
...
join #DeptHeirarchy dc on dc.DeptEnterpriseID = o.ChildOrganizationEID  
join #DeptHeirarchy dp on dp.DeptEnterpriseID = o.ParentOrganizationEID  
...
FROM         #DeptHeirarchy AS d1 WITH (nolock) 
					LEFT OUTER JOIN  #DPDOrgToOrg AS o1 WITH (nolock) ON d1.DeptEnterpriseID = o1.ChildOrganizationEID 
					  ...
					  LEFT OUTER JOIN  #DeptHeirarchy AS d2 WITH (nolock) ON d2.DeptEnterpriseID = o1.ParentOrganizationEID 
																							 
					  LEFT OUTER JOIN  #DPDOrgToOrg AS o2 WITH (nolock) ON o1.ParentOrganizationEID = o2.ChildOrganizationEID 
					  ...
					  LEFT OUTER JOIN  #DeptHeirarchy AS d3 WITH (nolock) ON d3.DeptEnterpriseID = o2.ParentOrganizationEID 
																							 
					  LEFT OUTER JOIN #DPDOrgToOrg AS o3 WITH (nolock) ON o2.ParentOrganizationEID = o3.ChildOrganizationEID 
					  ...
					  LEFT OUTER JOIN #DeptHeirarchy AS d4 WITH (nolock) ON d4.DeptEnterpriseID = o3.ParentOrganizationEID 
																							 
					  LEFT OUTER JOIN #DPDOrgToOrg AS o4 WITH (nolock) ON o3.ParentOrganizationEID = o4.ChildOrganizationEID 
					  ...
					  LEFT OUTER JOIN #DeptHeirarchy AS d5 WITH (nolock) ON d5.DeptEnterpriseID = o4.ParentOrganizationEID 
																							 
					  LEFT OUTER JOIN #DPDOrgToOrg AS o5 WITH (nolock) ON o4.ParentOrganizationEID = o5.ChildOrganizationEID 
					  ...
					  LEFT OUTER JOIN #DeptHeirarchy AS d6 WITH (nolock) ON d6.DeptEnterpriseID = o5.ParentOrganizationEID 
																						 
					  LEFT OUTER JOIN #DPDOrgToOrg AS o6 WITH (nolock) ON o5.ParentOrganizationEID = o6.ChildOrganizationEID 
					  ...
					  LEFT OUTER JOIN #DeptHeirarchy AS d7 WITH (nolock) ON d7.DeptEnterpriseID = o6.ParentOrganizationEID 
																							 
					 
	 
	 --SELECT count(*)
  --FROM [ETL_Local_PROD].[dbo].[DeptHierarchy]
  --SELECT count(*)      --from [ETL_Local_PROD].[dbo].[AT_PDToOrg] 
  --FROM  [ETL_Local_PROD].[dbo].[AT_Dept_Heirarchy]
 --select count(*) from [ETL_Local_PROD].[dbo].[AT_Dept] where DeptSource = 'PD'
  --select count(*) from PDDepartment
...
-- select top 5 * from #AuditPatient
--select top 5 * from  [ETL_Local_PROD].[dbo].[AuditPatient_New] 
  --select Count(*) from #AuditPatient
 --select top 5 * from [ETL_Local_PROD].[dbo].[AuditPatient_New] 
 --select  * from [ETL_Local_PROD].[dbo].[AuditPatient_New] where cmc_id =  100013863 and actiontype like '%view%' order by ActionTypeORder
...
--view [Reporting].[Cache-AccessDataDetail]
  ...
  if OBJECT_ID ('[ETL_Local_PROD].[dbo].[AT_AccessDataDetail]') is not null
	...
	join #DeptHeirarchy d1 on d1.DeptPDRegistryId = ap.DeptPDRegistryId
...
select top 10000 * from [ETL_Local_PROD].[dbo].[AT_AccessDataDetail] 
...
StaffEnterpriseId
from Reporting.AccessDataDetail pa with (nolock)
