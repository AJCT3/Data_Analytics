...
--select top 500 * from  [PatientDetailSpan]

--select   * from [ETL_Local_PROD].[dbo].[AT_AccessDataDetail] where cMC_ID = 100007061 order by CMC_ID, OverAllOrder
...
join #PD_Dept d on pa.ActivityDepartmentId = d.DeptLocalCMCId
left join Protocol.PatientDemographics demo on demo.CMC_ID = pa.cmc_id
left join #PD_Dept d2 on cast(demo.Original_Workbase_Id as varchar(25)) = d2.DeptLocalCMCId
left join (select cmc_id,ccg from [ETL_Local_PROD].[dbo].[AT_Patient_General]) pd on pd.CMC_ID = pa.cmc_id
left join  [ETL_Local_PROD].[dbo].[AT_Staff] s on pa.PrimaryStaffId = s.StaffLocalCMCId
-- Exclude view and revise activities by originating team on day of origination
