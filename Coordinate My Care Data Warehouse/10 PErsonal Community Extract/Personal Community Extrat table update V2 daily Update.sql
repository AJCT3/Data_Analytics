...
set @StartDate = (select min(submitTime) from [ETL_PROD].[dbo].[CMC_PersonalCommunityRequest])
...
-- select convert(date,dateadd(week,datediff(week,0,'2020-04-11'),0)) as WeekStart
 
-- SELECT DATEADD(DAY, 2 - DATEPART(WEEKDAY, '2020-04-18'), CAST('2020-04-18' AS DATE))
----select  DATEADD(dd, -(DATEPART(dw, '2020-04-13')-1), '2020-04-13') 
--select DATEADD(dd, -(DATEPART(dw, '2020-04-12')-1), '2020-04-12') 
--declare @Date datetime
--set @Date = '2020-04-12'
--select @Date - DATEPART(dw, @Date) + 1
  ...
  where [Calendar Day] >= @StartDate and [Calendar Day] < @EndDate
	...
	--select * from #Temp_CTE_Date order by mydate desc
