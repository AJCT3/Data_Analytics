...
/****** Object:  View [etl].[vw_ClaimGeneral]    Script Date: 09/04/2019 10:03:01 ******/
SET ANSI_NULLS ON
GO
...
AS
	...
	END as [Successful/Unsuccessful],  	--NEw Field	
        ...
        --P.CmsPostId					AS TeamPostId,
        --ISNULL(P.TeamName, 'Other') AS TeamPost,
        CAST(
            CASE
		...
		(SELECT MAX(Foo) FROM
			(
				...
				--SELECT Foo = CAST(r.Timestamp AS BIGINT) UNION ALL
	...
	----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	-- added  the rowNum after finding duplicate 22/02/2019 - a Dorset issue
    LEFT OUTER JOIN (
      select  d.* from  (
		SELECT   CC1.ClaimId, ROW_NUMBER() OVER(PARTITION BY CC1.ClaimId,CC1.StartDate Order by CC1.claimid,CC1.StartDate desc) as RowNum, CC1.ContactRef, O.OrgName, C1.FirstName, C1.Surname, C1.ContactId as DefenceSolicitorId,  
		(SELECT MAX(Foo) FROM
			(
    ...
    ) AS DefenceSolicitor ON  DefenceSolicitor.ClaimId = C.ClaimId 
	...
	--(
	--SELECT  
	--	c.FirstName+' '+c.Surname as Handler
	--	,a.[CMSPostId]
 --     ,a.[CMSUserId]
 --     ,[JobPercentage]
	--	,b.TeamName
	--	,b.TeamPost
	--	,a.Timestamp
 -- FROM [ClaimsManagement].[dbo].[CMSPostUser]a
 -- left join  [ClaimsManagement].[dbo].[CMSPost]b on b.CMSPostId = a.CMSPostId
 -- left join [ClaimsManagement].[dbo].[CMSUser]c on c.CMSUserId = a.CMSUserId
	--) as  P
 --       ON	P.CMSUserId=  C.HandlerId
